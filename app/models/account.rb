class Account < ApplicationRecord
  ...
  def spent_between(from_date, to_date, spend: nil, to_currency: nil, kinds: Transaction.kinds.keys)
    _t = transactions.between(from_date, to_date).not_personal_transfer.where('amount < 0')
    _t = _t.where(kind: kinds)
    _t = _t.where(spend: spend) if spend

    sum_to_currency(_t.sum(:amount), to_currency: to_currency)
  end

  def made_between(from_date, to_date, spend: nil, to_currency: nil, kinds: Transaction.kinds.keys)
    _t = transactions.between(from_date,to_date).not_personal_transfer.where('amount > 0')
    _t = _t.where(kind: kinds)
    _t = _t.where(spend: spend) if spend
